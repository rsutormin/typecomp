...
#   $options is a key/value hash of options, although currently no options yet exist and this parameter may be removed
#     in the future if it is determined that no options will be needed.
#
...
sub assemble_annotations
{
    ...
    # process annotations of type definitions
...
sub process_typedef_annotation_optional {
    ...
    ## first make sure we are point to a typedef of a structure, otherwise @optional makes no sense
